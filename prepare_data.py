...
import pandas as pd
from desc_tokenizer import feature_tokenize, description_generation_string,description_index
...
class PrepareData:
    def __init__(self):
        ...
        # 构建单词表
        self.en_word_dict, self.en_total_words, self.en_index_dict = self.build_dict_1(self.train_en)
    ...
    def load_data(self, path):
        ...
        for i in range(d1.shape[0]):
            source = feature_tokenize(description_generation_string(d1, i, Delimiter))
            if source != ['']:
                en.append(["BOS"] + source + ["EOS"])
            target = description_index(d1, i)
            if target != [''] and target != []:
                cn.append(["BOS"] + target + ["EOS"])
    ...
    def build_dict_1(self, sentences, max_words = 50000):
        word_count = Counter()
        ...
        print(len(word_dict))
        # np.save('/Users/wangyihao/Pycharm/3.npy',word_dict)
